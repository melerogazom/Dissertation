...
clf = joblib.load(f'co_activity_classifier_{current_exercise}.pkl')
...
exercise_clf = joblib.load("exercise_type_classifier_handcrafted.pkl")
...
def make_prediction():
    ...
    if len(buffer) >= MIN_WINDOW_SIZE:
        ...
        # Use handcrafted features instead of signature
        features = compute_handcrafted_features(latest_segment)

        clf_path = f'co_activity_classifier_{current_exercise}.pkl'
        if os.path.exists(clf_path):
            clf = joblib.load(clf_path)
            rep_pred = clf.predict(features)[0]
            ...
            if rep_pred == 1 and last_prediction == 0 and now - rep_detected_time > rep_timeout:
                ...
                # Exercise type prediction using handcrafted classifier
                exercise_pred = exercise_clf.predict(features)[0]
