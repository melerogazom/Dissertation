...
# ==== Load Data ====
CSV_FILE = "handcrafted_training_data.csv" # CSV file containing feature vectors and labels
df = pd.read_csv(CSV_FILE) # Load the data into a pandas DataFrame
...
min_samples = 5 # Minimum number of samples required per exercise type

for exercise in exercise_types:
    exercise_df = df[df['Exercise_Type'] == exercise] # Filter rows for this specific exercise
    
    # Skip exercises with too few samples
    if exercise_df.shape[0] < min_samples:
        # Not enough samples to train a valid model — skip this exercise
    ...
    model_filename = f'co_activity_classifier_{exercise}.pkl'
