...
def plot_f1_scores(signature_csv, handcrafted_csv):
    ...
    print(f"Processing {common_exercises[3]}...")  # For debugging
        
    # Evaluate signature and handcrafted methods for each exercise
    train_sizes_sig, f1_scores_sig, label_sig = evaluate_classifier(signature_csv, "Signature Methods", common_exercises[3])
    train_sizes_hand, f1_scores_hand, label_hand = evaluate_classifier(handcrafted_csv, "Handcrafted Methods", common_exercises[3])
    ...
    plt.ylabel("F1 Score")
    plt.title(f"Comparison of Signature and Handcrafted Methods for {common_exercises[3]}")
    plt.legend()
    plt.grid()
    plt.show()
